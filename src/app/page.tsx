...
export default function Home() {
  ...
  const handleLogin = async () => {
    try {
      const optionsRes = await axios.post(`${backend_url}/login_start`, { username });
      const options = await optionsRes.data;

      const authResponse = await startRegistration({optionsJSON: options.challenge.publicKey});

      const verificationRes = await fetch(`${backend_url}/login_finish`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  return (
    <div className="w-full min-h-screen flex flex-col items-center justify-center p-4 bg-gray-50">
      <div className="w-full max-w-md bg-white rounded-lg shadow-lg p-6">
        <h1 className="text-3xl font-bold text-center mb-6">Passkey Demo App</h1>
        ...
        <div className="space-y-4">
          ...
          >
            {activeTab === 'register' ? 'Register with Passkey' : 'Login with Passkey'}
          </button>
